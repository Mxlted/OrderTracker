...
public sealed class Order : ObservableObject
{
    ...
    private decimal? _projectedProfitOverride;
    ...
    public decimal? ProjectedProfitOverride
    {
        get => _projectedProfitOverride;
        set
        {
            var normalized = value.HasValue ? Math.Max(0m, value.Value) : (decimal?)null;
            SetProperty(ref _projectedProfitOverride, normalized);
        }
    }
